...
radars = [feat["geometry"]["coordinates"] for feat in geojson["features"]]
...
for feature in geojson["features"]:
    ...
    if radar_info["site_id"] is None or radar_info["elevation_ft"] is None:
        print(f"Skipping feature with missing SITE ID or elevation: {feature}")
        continue
    ...
    x = int((radar_info["easting"] - DEM_MIN_X) / dem_pixel_size_x) - window_size // 2
    y = int((DEM_MAX_Y - radar_info["northing"]) / dem_pixel_size_y) - window_size // 2
